...
| OAuth login CSRF, excess consent, or token disclosure | Login uses random state while storing only a domain-separated HMAC check in an HttpOnly, SameSite=Lax, ten-minute cookie; production adds `Secure`, callback comparison is timing-safe, and the callback clears the cookie. It does not request additional personal-information scopes for unimplemented profile features. Token exchange has a five-second abort timeout and schema validation, and the HTTP response omits access and refresh tokens. | The HMAC check is not server-side or replay-tracked. Replace it with an atomic, one-time TTL store before production use. |
